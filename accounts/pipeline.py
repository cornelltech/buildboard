...
def validate_cornell_email(backend, details, response, *args, **kwargs):
    cornell_regex = re.compile("^([A-Za-z0-9._%+-])+@cornell.edu$")
    new_school_regex = re.compile("^([A-Za-z0-9._%+-])+@newschool.edu$")
    if cornell_regex.match(details.get('email')) is None and new_school_regex.match(details.get('email')) is None:
